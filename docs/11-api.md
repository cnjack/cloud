...
model 持有上游 model id、context window、capabilities 与 Project grants。
...
| 端点 | 角色 | 说明 |
|---|---|---|
| `GET /api/v1/system/model-providers` | cluster-admin | provider 全量，内嵌 models、capabilities、`granted_project_ids`、聚合 `project_grants`；只回 `api_key_set`，绝不含明文 key |

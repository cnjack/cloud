...
```json
{
  ...
  "runner":    {
    "image": "ghcr.io/acme/runner:v1",   // RUNNER_IMAGE
    "persistent_workspace": true,        // PERSISTENT_WORKSPACE
    "prewarm": {                         // runner 镜像预下载 DaemonSet(jcloud-runner-prewarm)
      "supported": true,                 // launcher 具备 k8s 能力;process/disabled 时为 false
      "desired": 3, "ready": 2,          // DaemonSet 期望/就绪 pod 数(0/0 + last_sync 空 = 从未同步)
      "image": "ghcr.io/acme/runner:v1", // DaemonSet 模板上的实际镜像
      "last_sync": "2026-07-16T01:00:00Z" // 上次经 API 触发同步的时间;空 = 从未同步
      // 状态读取失败时改为 "error": "<curated line>"(绝不带原始 client-go 错误细节)
    }
  },
...
#### `POST /api/v1/system/runner-image/prewarm` — 同步/预下载 runner 镜像(cluster-admin)

console Cluster 页"同步最新镜像"按钮的触发端点。orchestrator 维护一个
`jcloud-runner-prewarm` **DaemonSet**(每节点一个 `sleep infinity` 的 sleeper pod,
镜像 = 当前 `RUNNER_IMAGE`,`imagePullPolicy: Always`,资源 floor 1m/16Mi),
让每个节点常驻缓存 runner 镜像,run 首次启动不再付冷拉取成本。一次 POST:

1. DaemonSet 不存在 → 创建;
2. `RUNNER_IMAGE` 已变化 → 更新模板(controller 自行滚动);
3. 删除现有 prewarm pod —— 覆盖"同 tag 重推"(`:latest`)场景:模板不变不会
   滚动,只能靠重建 pod + `PullAlways` 把新 digest 拉下来。

响应 `200` = 同步后的 prewarm 快照(同 `GET /system` 的 `runner.prewarm`)。
错误:`401`;`403`(非 cluster-admin);`409 prewarm_not_supported`
(launcher 为 process/disabled,无集群可预拉——fail-visible,不假装成功);
`500`(K8s 调用失败)。需要 RBAC:`apps/daemonsets` create+get+update,
`pods` delete(见 `deploy/base/orchestrator/rbac.yaml`)。
...
### 2.5a 模型目录 + Account / Project 授权(D21)
...
| `GET /api/v1/system/model-providers/{id}/catalog` | cluster-admin | 精确命中 Desktop provider kind 时直接读取构建期 models.dev 完整目录（`metadata_source=models.dev`），不让上游 `/models` 的缺失或凭据策略阻断浏览；未知/自定义 kind 才读取实时 `/models`；`catalog_mode=disabled` 时 `409 catalog_unavailable` |
...
Models 页面以 `/system/model-providers` 为管理入口。精确命中 Desktop provider kind 时，
provider catalog 以 `internal/modelmetadata` 中由 `go generate` 从 models.dev 生成的快照
作为完整、确定性的集合，不请求上游 `/models`；连接验证仍由 `POST .../verify` 单独实时
执行。未知/自定义 kind 才以上游实时返回的 model id 为集合边界，且未知元数据保持空名称、
零 context 与 false capabilities，不按名称猜测。display name、context window 与
capabilities 在配置 model 时显式落库；已有 catalog model 只有管理员点击“同步元数据”
才会被快照覆盖，人工修正不会在 GET 时被静默改写。
...
| `POST /api/v1/projects/{id}/kanban/links` | owner | `{workspace_id, board_ref, service_id, trigger_column, done_column?, token?}`;`service` 须属本 project(否则 `400`);`token` 可选、**只写**(明文进、绝不回);集成开且传了 token 时用该 token 对 live board 校验列名(D30),否则软建 `board_status="unvalidated"` |
| `PATCH /api/v1/projects/{id}/kanban/links/{linkId}` | owner | **只轮换/清除 token**:body `{"token":"..."}`(字段必发,缺省 `400`);`""` = 清除(link 转 `missing`),非空 = 轮换;**claims 保留**(轮换绝不重派已认领卡片);link 须属本 project(否则 `404`);cipher 缺失 `409`(同 create) |
...
- 集成开且传了 `token` → 对 live board 硬校验:`trigger_column`/`done_column`
  不是真实列 → `400`;jtype 不可达 → `503 jtype_unreachable`;board 找不到/歧义 →
...
(`"per_link" | "missing"`,D36 起两态;`missing` = poller/回写 fail-visible
跳过该 link,console 以醒目错误徽标呈现),**永不含 token**。

**运行期 token 选取(poller 拉取 + reconciler 回写 + 看板嵌入代理 + discovery
共用)**:`token_enc` 非空 → 解密用之;为空 → 该 link **fail-visible 跳过**(不以
空凭据打 jtype),回写侧则把该卡的 writeback **挂起重试**(不丢结果),owner 补上
token 后自动恢复。
...
`GET /api/v1/system` 的 `kanban` 字段:`enabled`、`base_url`、`poll_interval`、

...
| POST | `/internal/v1/device/sessions` | upsert 会话索引：`{sessions:[{session_id, status:"idle"\|"running", meta}], replace:true}`（meta 原样存取——M3 明文 JSON，M5 起为密文，服务端不解析）；`replace:true` 将其作为设备完整快照并删除未列出的 mirror/events（本地删除或关闭同步会收敛到云端）；响应 `{sessions:[{session_id, last_seq}]}`（该 session 当前最大 durable seq，无则 0，供重连续号） |
...
指令 kind：`chat.send`（新会话/追加消息）、`chat.stop`、`approval.respond`、`session.delete`、`workspace.browse`（设备侧只读目录浏览）、`session.list.req`、`session.patch`（pin/archive/title）、`pairing.request`（§6.3）。
...
| DELETE | `/api/v1/devices/{id}/sessions/{sid}` | 入队 `session.delete` 并立即移除云端 mirror；设备下一次完整快照会在本地删除失败时恢复 mirror；E2EE 设备需要 `{envelope}` |
...
- **门**：所有客户端下行指令端点（messages、stop、approval、session delete、workspace browse）在 `e2ee=true` 时只接受 `{envelope:{enc,...}}` 密文形式；明文 body 返回 **409 `{error:{code:"pairing_required"}}`**，不入队。客户端见此错误应引导配对（§6.3）拿 CEK 后改发密文。

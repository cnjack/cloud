...
状态：Draft（M3 — 登录/注册/relay 明文已实现；E2EE 未上线）
...
| POST | `/api/v1/devices/{id}/sessions/{sid}/messages` | 发消息/新会话：`{text, mode?}`，sid 可为 `new`（指令 session_id=null）；入队 `chat.send`（payload 加 `channel:"console"`），202 `{command_id, session_id}`；设备 offline → 409 `device_offline` |
| POST | `/api/v1/devices/{id}/sessions/{sid}/stop` | 入队 `chat.stop`（payload `{}`，目标在指令 session_id 字段）；offline 同样 409 |
| POST | `/api/v1/devices/{id}/sessions/{sid}/approval` | `{approval_id, decision}` → 入队 `approval.respond`；offline 同样 409 |
| POST | `/api/v1/devices/{id}/pairings` | 发起配对请求（§6.3，P3） |
| GET | `/api/v1/devices/{id}/pairings` | 配对状态查询（P3） |

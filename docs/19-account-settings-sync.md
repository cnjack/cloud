...
Desktop 可见集合为用户有权访问的 Project enabled models 与 Cluster grants
的并集。以后新增 account-scoped Cloud Provider 时沿用同一 DTO，不改变
Desktop 聚合协议。
...
3. 证明该用户对 model 所属 project/cluster grant 有访问权；
...
1. Desktop 只看见用户有权访问且 enabled/granted 的模型。
2. DTO `kind` 驱动与 Desktop 相同的 icon。
3. device token 永不转发上游；真实 key 最后注入。
4. custom headers 过滤 Host/hop-by-hop，受管 Authorization 胜出。
5. SSE、普通响应、GET `/models` 都可透传。
6. 跨项目 model UUID 与未知 UUID 都返回相同的 404，不泄露配置存在性。
7. Cloud 不可用时 Desktop 本地 provider 仍工作。

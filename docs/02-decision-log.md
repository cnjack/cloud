...
---
### D36 · 砍掉集群级 jtype 回退凭据:集群 kanban 配置只剩 base URL,凭据完全 per-link

集群级 jtype 配置收敛为**只有 base URL**(基础设施事实:本集群连哪个 jtype 实例),彻底移除 cluster fallback token(DB `cluster_kanban_config.token_enc/token_expires_at` + env `JTYPE_TOKEN` + cluster 面设备流)。理由:D25 已把凭据下放到 per-link,fallback token 只剩"deprecated 兜底"角色(poller/writeback 每次用都打 deprecation warn),却拖着一整套同源绑定、cipher 纪律、expiry 展示的复杂度;而且集群里就一个 jtype 实例,多实例才需要的"按 project 配 base URL"被明确否决(那是方案 B,不做)。

- **移除面**:`domain.KanbanConfig` 不再含 token 字段;`kanbancfg.Resolver` 不再持 cipher,`Factory()` 只回 `(factory, ok)`;`jtype.ResolveToken` 从三态(per-link > cluster fallback > ErrNoToken)收成两态(per-link > ErrNoToken);`credential_status` 从三态收成 `"per_link" | "missing"`;`PUT /system/kanban` body 只剩 `base_url`;`POST/GET /system/kanban/connect*`(cluster 面设备流)与 store 的 `SetClusterKanbanToken` 一并删除。迁移 `0042` `DROP COLUMN IF EXISTS`(幂等),存量集群回退 token 被**丢弃**——靠它跑的 link 转 `missing` fail-visible,owner 补 per-link token 即恢复,绝不静默。
- **不变面**:base URL 仍 cluster 级(DB override > `JTYPE_BASE_URL` env,D27 的 resolver/Invalidate/TTL 机制原样);poller 每 tick 解析、集成 off 即整体可见 no-op;per-link token 的手贴/PATCH 轮换/设备流 Connect(D28 link 面)、D30 软建+运行时复核、D31 看板嵌入代理全部照旧。

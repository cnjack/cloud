...
await sleep(3500);
console.log('stop enabled while running:', evaluate(`!document.querySelector('[data-testid="stop-session"]').disabled`));
overflow('streaming');
shot('11-session-streaming');
await waitFor(`document.querySelector('[data-testid="stop-session"]').disabled`, { timeoutMs: 90_000, label: 'turn finished' });
await sleep(1000);

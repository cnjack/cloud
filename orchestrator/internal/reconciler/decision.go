...
// shouldOpenPR is the PURE decision for the draft-PR flow (M3): given a run and
// its service, should the reconciler push + open a draft PR this tick? It is
// separated from the side-effecting push/provider calls so it is exhaustively
// unit-tested with no store/provider/HTTP.
//
// The gate: the run must be a succeeded AGENT run, the service must be draft_pr
// mode on ANY provider (gitea/github/gitlab) with a provider repo (owner/name)
// configured, a branch must be recorded (git_branch, stamped when the bundle was
// received), and no PR may exist yet (pr_url empty). `providerReady` lets the
// caller decline when the draft-PR stack is not configured. Any false is a clean
// skip.
